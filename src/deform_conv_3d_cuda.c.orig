...
#include "deform_conv_3d_cuda_kernel.h"

extern THCState *state;

void shape_check(THCState *state, THCudaTensor *input, THCudaTensor *offset,
                 THCudaTensor *gradOutput, THCudaTensor *weight, int kH, int kW, int kD,
                 int dH, int dW, int dD, int padH, int padW, int padD, int dilationH,
                 int dilationW, int dilationD, int deformable_group) {

//  THArgCheck(weight->nDimension == 4, 5,
//             "4D weight tensor (nOutputPlane,nInputPlane,kH,kW) expected, "
//             "but got: %s",
//             weight->nDimension);
  THArgCheck(THCudaTensor_nDimension(state, weight) == 5, 5,
             "5D weight tensor (nOutputPlane,nInputPlane,kH,kW,kD) expected, "
             "but got: %s",
  ...
  THArgCheck(kW > 0 && kH > 0 && kD > 0, 11,
             "kernel size should be greater than zero, but got kH: %d kW: %d kD: %d",
             kH, kW, kD);
...
//             kW, weight->size[2], weight->size[3]);
  THArgCheck((THCudaTensor_size(state, weight, 2) == kH &&
             THCudaTensor_size(state, weight, 3) == kW && 
             THCudaTensor_size(state, weight, 4) == kD), 11,
             "kernel size should be consistent with weight, ",
             "but got kH: %d kW: %d kD: %d weight.size(2): %d, weight.size(3): %d, weight.size(4): %d", kH,
             kW, kD, THCudaTensor_size(state, weight, 2), THCudaTensor_size(state, weight, 3), THCudaTensor_size(state, weight, 4));


  THArgCheck(dW > 0 && dH > 0 && dD > 0, 11,
             "stride should be greater than zero, but got dH: %d dW: %d dD: %d", dH, dW, dD);

  THArgCheck(dilationW > 0 && dilationH > 0 && dilationD > 0, 18,
      "dilation should be greater than 0, but got dilationH: %d dilationW: %d dilationD: %d",
      dilationH, dilationW, dilationD);
...
//  long nOutputPlane = weight->size[0];
  ...
  long inputDepth = THCudaTensor_size(state, input, dimd);
  ...
  if (outputWidth < 1 || outputHeight < 1 || outputDepth < 1)
    THError(
        ...
        nInputPlane, inputHeight, inputWidth, inputDepth, nOutputPlane, outputHeight,
  ...
  THArgCheck((inputHeight >= kH && inputWidth >= kW && inputDepth >= kD), 2,
...
//      offset->size[2], offset->size[3]);
  THArgCheck(
      ...
      THCudaTensor_size(state, offset, 3) == outputWidth &&
      THCudaTensor_size(state, offset, 4) == outputDepth), 3,
      "invalid spatial size of offset, expected height: %d width: %d depth: %d, but got height: %d width: %d depth: %d",
      outputHeight, outputWidth, outputDepth, THCudaTensor_size(state, offset, 2),
      THCudaTensor_size(state, offset, 3), THCudaTensor_size(state, offset, 4));

  THArgCheck((THCudaTensor_size(state, offset, 1) == deformable_group * 2 * kH * kW * kD), 3,
  ...
  if (gradOutput != NULL) {
    ...
    THArgCheck((THCudaTensor_size(state, gradOutput, dimh) == outputHeight &&
                ...
                THCudaTensor_size(state, gradOutput, dimd) == outputDepth),
               4, "invalid size of gradOutput, expected height: %d width: %d depth: %d , but got height: %d width: %d depth: %d",
               outputHeight, outputWidth, outputDepth, THCudaTensor_size(state, gradOutput, dimh),
               THCudaTensor_size(state, gradOutput, dimw), THCudaTensor_size(state, gradOutput, dimd));
  }
}
...
int deform_conv_forward_cuda(THCudaTensor *input, THCudaTensor *weight,
  ...
  shape_check(state, input, offset, NULL, weight, kH, kW, kD, dH, dW, dD, padH, padW, padD,
              dilationH, dilationW, dilationD, deformable_group);
  ...
  if (THCudaTensor_nDimension(state, input) == 4) {
    // Force batch
    batch = 0;
    THCudaTensor_resize5d(state, input, 1, THCudaTensor_size(state, input, 0), THCudaTensor_size(state, input, 1),
                          THCudaTensor_size(state, input, 2), THCudaTensor_size(state, input, 3));
    THCudaTensor_resize5d(state, offset, 1, THCudaTensor_size(state, offset, 0), THCudaTensor_size(state, offset, 1),
  ...
  long inputDepth = THCudaTensor_size(state, input, 4);
  ...
  THCudaTensor_resize5d(state, output, batchSize / im2col_step, im2col_step, nOutputPlane, outputHeight, outputWidth, outputDepth);
  THCudaTensor_resize2d(state, columns, nInputPlane * kW * kH * kD, im2col_step * outputHeight * outputWidth * outputDepth);

  if (THCudaTensor_nDimension(state, ones) != 2 || THCudaTensor_size(state, ones, 0) *
      THCudaTensor_size(state, ones, 1) < outputHeight * outputWidth * outputDepth) {
    THCudaTensor_resize2d(state, ones, outputHeight, outputWidth, outputDepth);
  ...
  THCudaTensor_resize5d(state, output_buffer, batchSize / im2col_step, nOutputPlane, im2col_step * outputHeight, outputWidth, outputDepth);

  THCudaTensor_resize6d(state, input, batchSize / im2col_step, im2col_step, nInputPlane, inputHeight, inputWidth, inputDepth);
  THCudaTensor_resize6d(state, offset, batchSize / im2col_step, im2col_step,
      deformable_group * 3 * kH * kW * kD, outputHeight, outputWidth, outputDepth);
  ...
  for (int elt = 0; elt < batchSize / im2col_step; elt++) {
    ...
    deformable_im2col(
        ...
        inputWidth, inputDepth, kH, kW, kD, padH, padW, padD, dH, dW, dD, dilationH, dilationW, dilationD,
  ...
  THCudaTensor_resize6d(state, output_buffer, batchSize / im2col_step, nOutputPlane, im2col_step, outputHeight, outputWidth, outputDepth);
  THCudaTensor_transpose(state, output_buffer, NULL, 1, 2);
  THCudaTensor_copy(state, output, output_buffer);
  THCudaTensor_resize5d(state, output, batchSize, nOutputPlane, outputHeight, outputWidth, outputDepth);

  THCudaTensor_resize5d(state, input, batchSize, nInputPlane, inputHeight, inputWidth, inputDepth);
  THCudaTensor_resize5d(state, offset, batchSize, deformable_group * 3 * kH * kW * kD, outputHeight, outputWidth, outputDepth);
...
int deform_conv_backward_input_cuda(
  ...
  shape_check(state, input, offset, gradOutput, weight, kH, kW, kD, dH, dW, dD, padH,
              padW, padD, dilationH, dilationW, dilationD, deformable_group);
  ...
  if (THCudaTensor_nDimension(state, input) == 4) {
    ...
    THCudaTensor_resize5d(state, offset, 1, THCudaTensor_size(state, offset, 0), THCudaTensor_size(state, offset, 1),
                          THCudaTensor_size(state, offset, 2), THCudaTensor_size(state, offset, 3));
    THCudaTensor_resize5d(state, gradOutput, 1, THCudaTensor_size(state, gradOutput, 0),
                          THCudaTensor_size(state, gradOutput, 1), THCudaTensor_size(state, gradOutput, 2), THCudaTensor_size(state, gradOutput, 3));
  }
  ...
  THCudaTensor_resize6d(state, gradOutput, batchSize / im2col_step, im2col_step, nOutputPlane, outputHeight, outputWidth, outputDepth);
  THCudaTensor_transpose(state, gradOutput, NULL, 1, 2);

  THCudaTensor *gradOutputBuffer = THCudaTensor_new(state);
  THCudaTensor_resize6d(state, gradOutputBuffer, batchSize / im2col_step, nOutputPlane, im2col_step, outputHeight, outputWidth, outputDepth);
  THCudaTensor_copy(state, gradOutputBuffer, gradOutput);
  THCudaTensor_resize5d(state, gradOutputBuffer, batchSize / im2col_step, nOutputPlane, im2col_step * outputHeight, outputWidth, outputDepth);

  THCudaTensor_transpose(state, gradOutput, NULL, 1, 2);
  THCudaTensor_resize5d(state, gradOutput, batchSize, nOutputPlane, outputHeight, outputWidth, outputDepth);

  THCudaTensor_resize6d(state, gradInput, batchSize / im2col_step, im2col_step, nInputPlane, inputHeight, inputWidth, inputDepth);
  THCudaTensor_resize6d(state, input, batchSize / im2col_step, im2col_step, nInputPlane, inputHeight, inputWidth, inputDepth);
  THCudaTensor_resize6d(state, gradOffset, batchSize / im2col_step, im2col_step,
      deformable_group * 3 * kH * kW * kD, outputHeight, outputWidth, outputDepth);
  ...
  for (int elt = 0; elt < batchSize / im2col_step; elt++) {
    ...
    deformable_col2im_coord(
        ...
        dilationH, dilationW, dilationD, im2col_step, deformable_group,
        THCudaTensor_data(state, gradOffset_n));

    deformable_col2im(
        THCState_getCurrentStream(state), THCudaTensor_data(state, columns),
        THCudaTensor_data(state, offset_n), nInputPlane, inputHeight,
        inputWidth, inputDepth, kH, kW, kD, padH, padW, padD, dH, dW, dD, dilationH, dilationW, dilationD, im2col_step,
        deformable_group, THCudaTensor_data(state, gradInput_n));
  }

  THCudaTensor_resize5d(state, gradInput, batchSize, nInputPlane, inputHeight, inputWidth, inputDepth);
  THCudaTensor_resize5d(state, input, batchSize, nInputPlane, inputHeight, inputWidth, inputDepth);
  THCudaTensor_resize5d(state, gradOffset, batchSize, deformable_group * 3 * kH * kW * kD, outputHeight, outputWidth, inputDepth);
  THCudaTensor_resize5d(state, offset, batchSize, deformable_group * 3 * kH * kW * kD, outputHeight, outputWidth, outputDepth);
  ...
  if (batch == 0) {
    THCudaTensor_resize4d(state, gradOutput, nOutputPlane, outputHeight,
                          outputWidth, outputDepth);
    THCudaTensor_resize4d(state, input, nInputPlane, inputHeight, inputWidth, inputDepth);
    THCudaTensor_resize4d(state, gradInput, nInputPlane, inputHeight,
                          inputWidth, inputDepth);
    THCudaTensor_resize4d(state, offset, THCudaTensor_size(state, offset, 1), THCudaTensor_size(state, offset, 2),
                          THCudaTensor_size(state, offset, 3), THCudaTensor_size(state, offset, 4));
    THCudaTensor_resize4d(state, gradOffset, THCudaTensor_size(state, offset, 1), THCudaTensor_size(state, offset, 2),
                          THCudaTensor_size(state, offset, 3), THCudaTensor_size(state, offset, 4));
  }
...
int deform_conv_backward_parameters_cuda(
  ...
  shape_check(state, input, offset, gradOutput, gradWeight, kH, kW, kD, dH, dW, dD,
             padH, padW, padD, dilationH, dilationW, dilationD, deformable_group);
  ...
  THCudaTensor_resize2d(state, columns, nInputPlane * kW * kH * kD,
                        im2col_step * outputHeight * outputWidth * outputDepth);
